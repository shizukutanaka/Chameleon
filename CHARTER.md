...
**Q (2026-09-22, continued):** Audit-106 fixed the gain bounds in the
committed `simple_gain` demo -- but the demo was generated by
`_generate_plugin_template`. Does the generator still emit the same
dishonest plugin for every new author who scaffolds from it?
**A:** Worse than just the unchecked gain: the template stamped a `gain`
parameter into every category's metadata -- analyzers and utilities
that never read it, and the generator whose example reads `frequency`
instead (declared-but-unread and consumed-but-undeclared in one file),
plus an unchecked `gain` application in the effect example -- the
audit-106 bug at its source. It also fell into the utility branch for
any unknown category while stamping that category in the metadata --
`create_plugin_template("X", "bogus")` generated a plugin its own
`_validate_plugin` refuses to load. Parameters are now per-category
(effect: gain, enforced in the example; generator: frequency 20-20000Hz,
declared and enforced; analyzer/utility: none) and unknown categories
are refused up front. Verified: all four templates still load through
the real sandbox+validator path.
