...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 the audit-46 fixes were reverted
  the same way — `core.py` restored to the double-appending, literal-glob
  version — and the suite re-run: five of the six new tests caught it
  (the idle-run stabilisation pin asserts unchanged behaviour, so it
  passes either way).
