...
40 skipped), **564** with numpy (scipy/librosa/soundfile blocked, 33
...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 the audit-45 fix was reverted the
  same way — `audio_restoration.py` restored to the un-gated version — and
  the suite re-run: all seven new subprocess tests caught it (NameError on
  `signal` instead of the documented RuntimeError).
