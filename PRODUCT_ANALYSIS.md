...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  branch's four tests: the empty-input restore, both .WAV discovery tests, and
  the tag-order test all failed against the un-fixed source (the last under
  three distinct PYTHONHASHSEED values).
