...
33 skipped), **559** with numpy (scipy/librosa/soundfile blocked, 33
...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 a seventh re-verification (audit 57)
  found the vacuous suite was not the only fake check in the repo:
  `validation_test.py`, the third step of the documented verification gate,
  exercised `tempfile` and `struct` while importing no product code at all;
  it now runs `core.analyze` and `SecurityValidator` for real (verified by
  sabotage: patching `analyze` or `validate_audio_content` makes it fail),
  and `test_validation_gate_is_real.py` guards the regression.
