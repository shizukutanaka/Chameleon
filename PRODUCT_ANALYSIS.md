...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 three plugin-sandbox tests on this
  branch were mutation-verified: the sub-second-timeout, bounded-cleanup, and
  bounded-getattr tests each failed when the fix was reverted.
