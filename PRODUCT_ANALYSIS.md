...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 the same check was run against this
  branch's seven new parallel-batch tests: six failed against the un-fixed
  source on the first pass; the threshold test also passed because one
  input file made the old per-op failure indistinguishable from an upfront
  refusal — re-armed with two files, it fails too. All seven now bite.
