...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. Re-checked 2026-09-22 (audit 44): the two new
  CLI-surface tests were re-run against the unfixed source — the
  unsafe-`--output-dir` test returned exit 1 instead of 4, and the
  uppercase-`.WAV` batch test found zero files — so both tests bite.
