...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  On 2026-09-22 the same revert-and-rerun check was applied to audit-54's
  fixes: restoring the N//4-bin DFT fails the 15 kHz peak test, restoring
  the ignored overlap fails the window-count test, and restoring the
  single-window mean-magnitude fails both flux tests — all four new tests
  bite.
