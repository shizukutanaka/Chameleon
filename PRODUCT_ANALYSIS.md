...
all three configurations — **466 passed** on a bare install (stdlib only,
33 skipped), **546** with numpy (scipy/librosa/soundfile blocked, 33
skipped), **651** with numpy + scipy + librosa + soundfile + fastapi
(5 skipped; two batch-poll tests flake under load and pass standalone —
a known timing issue, not a regression). Skip totals follow which extras are installed — e.g. the two
