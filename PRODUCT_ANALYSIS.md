...
all three configurations — **465 passed** on a bare install (stdlib only,
33 skipped), **545** with numpy (scipy/librosa/soundfile blocked, 33
skipped), **650** with numpy + scipy + librosa + soundfile + fastapi
(5 skipped; one batch-poll test flaked under load in the full run and
passes standalone — a known timing flake). Skip totals follow which extras are installed — e.g. the two
