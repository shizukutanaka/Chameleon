...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 a sixth re-verification (audit 56)
  added six more cases — five bite on revert (a mid-scan read failure that
  used to return a truncated "successful" trim, and loop-workflow
  `iterations` values that crashed or silently ran nothing); the sixth is a
  labeled contract pin (`iterations: 0` is an honest no-op either way).
