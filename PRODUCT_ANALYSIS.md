...
50 skipped), **703** with numpy (scipy/librosa/soundfile blocked, 44
skipped), **822** with numpy + scipy + librosa + soundfile + fastapi
...
| ~~P2~~ | ~~Every exported bs1770 meter crashed on ndarray input (`if not samples:`/`if not channels:` on a `Sequence` that is really an ndarray)~~ | Med | XS | Med | **DONE 2026-09-20** — guards now `len(...) == 0`; the tests' `tolist()` had masked it (`tests/test_loudness_ndarray_input.py`) |
...
| ~~P2~~ | ~~`Limiter` delay-line: zero prefix + dropped tail every call, previous buffer's audio emitted as next buffer's head; `Compressor` envelope leaked across calls; `ratio=0`/`release≈0`/NaN configs crashed or polluted~~ | High | M | Med | **DONE 2026-09-20** — forward-window lookahead, per-call state reset, config validation (`tests/test_processor_state_reset.py`) |
