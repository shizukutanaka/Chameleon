...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 both new checks on this branch were
  mutation-verified the same way: the mid-write abort's corrupt-partial-file
  leak and the mono downmixer's truncation bias, each failing against the
  reverted source.
