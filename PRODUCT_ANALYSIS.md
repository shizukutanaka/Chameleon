...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 three more tests on this branch were
  mutation-verified the same way: the same-stem batch-output collision test and
  both sanitizer size-honesty tests each failed when their fix was reverted.
