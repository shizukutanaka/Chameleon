...
- **The suite is mutation-checked, not just green.** "445 tests pass" says
  ...
  itself is the assertion. On 2026-09-22 all four new checks on this branch
  were mutation-verified the same way: the async batch's unnormalized op
  dispatch, its symlink-following scan, the raw SecurityError escape on an
  unsafe directory, and the batch-level bounds that disagreed with the
  per-op guards.
