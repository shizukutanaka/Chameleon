...
- `file_name` には `/audio/upload` の応答で受け取った `stored_name` を指定します。`http://` / `https://` の URL を指定した場合、エンドポイントは `400 Bad Request` を返します（リモート URL は処理対象外。先に `/audio/upload` でアップロードしてください）。
