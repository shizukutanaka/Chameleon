...
- **Base URL**: 各組織のリバースプロキシまたは API ゲートウェイで公開した HTTPS エンドポイントを使用します（慣例として `CHAMELEON_BASE_URL` 環境変数でクライアント側に指定します — サーバー自体はこの変数を読みません）。ローカル開発時は `http://localhost:8080`、本番環境では必ず HTTPS を使用してください。
