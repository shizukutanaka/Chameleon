...
_MIN_ITEMS_KW = "min_length" if _PYDANTIC_V2 else "min_items"
...
SECURITY_CONFIG = {
    ...
    # uploaded_files was the only unbounded registry in APIState: sessions,
    # jobs, the audit deque and rate-limit windows are all capped, but a
    # valid session could append upload entries (and write up to
    # max_file_size per entry) forever -- memory and disk exhaustion at
    # authenticated-request speed. Count and bytes are both bounded; the
    # byte budget is the disk-exhaustion control, the count the registry's.
    'max_uploaded_files': 1000,
    'max_uploaded_bytes': 5 * 1024**3,  # 5 GiB of stored uploads + outputs
...
class BatchJobRequest(BaseModel):
    # min_items: an empty file list used to create a job that processes
    # nothing and reports success -- an accepted-but-meaningless request.
    files: List[str] = Field(..., **{_MIN_ITEMS_KW: 1})
...
class APIState:
    def __init__(self):
        ...
        # Cumulative bytes of every file the registry tracks -- the only
        # accounting that exists for the upload directory on disk, so the
        # max_uploaded_bytes budget has a number to enforce against.
        self.uploaded_bytes_total: int = 0
    ...
    def register_uploaded_file(
        ...
        *,
        ...
        previous = self.uploaded_files.get(file_name)
        if previous is not None:
            self.uploaded_bytes_total -= int(previous.get('size') or 0)
        self.uploaded_bytes_total += size
...
def _enforce_uploaded_file_capacity(extra_bytes: int = 0) -> None:
    """Reject work that would exceed the tracked upload storage budget.

    uploaded_files is the only accounting the server keeps for the upload
    directory on disk; nothing removes entries or files, so without a bound
    both the registry (memory) and the WAVs behind it (disk) grew without
    limit under any valid session.
    """
    max_files = SECURITY_CONFIG.get('max_uploaded_files')
    if max_files and len(api_state.uploaded_files) >= max_files:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Upload storage is at capacity; try again later",
        )
    max_bytes = SECURITY_CONFIG.get('max_uploaded_bytes')
    if max_bytes and api_state.uploaded_bytes_total + extra_bytes > max_bytes:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Upload byte budget exhausted; try again later",
        )
    ...
    try:
        with _UPLOAD_FILES.secure_open(target_path, 'wb') as buffer:
            while True:
                ...
                if api_state.uploaded_bytes_total + total_bytes > \
                        SECURITY_CONFIG['max_uploaded_bytes']:
                    raise HTTPException(
...
async def upload_audio_file(
    ...
    try:
        ...
        _enforce_uploaded_file_capacity()
...
async def normalize_audio(
    ...
    try:
        ...
        try:
            estimated_output_bytes = input_path.stat().st_size
        except OSError:
            estimated_output_bytes = 0
        _enforce_uploaded_file_capacity(estimated_output_bytes)
...
async def process_batch_job(job_id: str):
    ...
    try:
        ...
        # active_jobs[job_id] raised KeyError before, and the outer except
        # then referenced the never-bound name -- a missing job surfaced as
        # UnboundLocalError instead of a clean early exit.
        if job_data is None:
            logging.error("Batch job %s disappeared before processing", job_id)
            return
        ...
        async with JOB_WORKER_SEMAPHORE:
            ...
            for i, file_name in enumerate(job_data['files']):
                ...
                try:
                    ...
                    if job_data['operation'] == 'analyze':
                        ...
                        try:
                            ...
                            try:
                                ...
                                estimated_bytes = 0
                            _enforce_uploaded_file_capacity(estimated_bytes)
                            result = await normalize_audio_fast(
                    ...
                    # A file that vanishes (or stops resolving) between
                    # submit and processing used to kill the whole job via
                    # the outer except: every remaining file silently got no
                    # result and the job reported 'failed'. Per-file
                    # failures are data about that file, not a job abort.
                    # asyncio.CancelledError is BaseException and still
                    # propagates, so job cancellation stays clean.
                    result = {'success': False, 'error': str(file_exc)}
            ...
            job_data['completed_at'] = datetime.now(timezone.utc)
            failed_count = sum(
                1 for entry in job_data['results']
                if not entry['result'].get('success'))
            if failed_count:
                job_data['error'] = (
                    f"{failed_count} of {job_data['total_files']} files failed")
